...
rng(42);
ecg_n = ecg + 0.15*sin(2*pi*0.3*t) + 0.08*sin(2*pi*NF*t) + 0.03*randn(N,1);
...
if exist([REC '.atr'], 'file')
    ...
    for k=1:nr
        [mn,mi]=min(abs(ann_disp-r_ecg(k)));
        if mn<=tol && ~ma(mi)
...
if has_ann
    plot(t(ann_disp), ecg_f(ann_disp), 'g^', 'MarkerFaceColor',[0 .8 0],'MarkerSize',9);
    plot(t(r_ecg),    ecg_f(r_ecg),   'rv', 'MarkerFaceColor','r','MarkerSize',8);
    legend('ECG da loc (IIR+Notch)', sprintf('Annotation .atr (%d beats, vi tri chinh xac)',numel(ann_s)), sprintf('Pan-Tompkins (%d beats)',nr), 'Location','ne');
    title(sprintf('ECG da loc | Se=%.1f%%  +P=%.1f%%  F1=%.1f%%  |  HR=%.1f+-%.1f bpm  |  SDNN=%.1fms  RMSSD=%.1fms', Se,PP,F1,mean(hr),std(hr),SDNN,RMSSD));
else
    plot(t(r_ecg),ecg_f(r_ecg),'rv','MarkerFaceColor','r','MarkerSize',8);
    title(sprintf('ECG da loc | %d dinh R  |  HR=%.1f+-%.1f bpm  |  SDNN=%.1fms  RMSSD=%.1fms', nr,mean(hr),std(hr),SDNN,RMSSD));
end
xlabel(xt); ylabel(yt); xlim(xl); grid on; box on;
...
fill([t_hr(1) t_hr(end) t_hr(end) t_hr(1)], [mean(hr)-std(hr) mean(hr)-std(hr) mean(hr)+std(hr) mean(hr)+std(hr)], 'r','FaceAlpha',0.1,'EdgeColor','none');
title(sprintf('Nhip tim tuc thoi  |  TB=%.1f bpm  |  SD=%.1f bpm  |  pNN50=%.1f%%', mean(hr),std(hr),pNN50));
...
function [r_mwi,r_ecg,S]=pan_tompkins(ecg,fs)
    ecg=ecg(:); N=numel(ecg);
    [bl,al]=butter(5,min(15/(fs/2),0.99),'low');
    ...
    for k=1:nr
        ...
        i1=min(N, double(rloc(k))+round(0.05*fs));
        [~,lm]=max(ecg(i0:i1));
    ...
    thr=zeros(N,1);
    if nr>=1
        thr=interp1([1;r_mwi;N],[thhist(1);thhist(r_mwi);thhist(N)],(1:N)','linear','extrap');
        thr=max(thr,0);
    end
